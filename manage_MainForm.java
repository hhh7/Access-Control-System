...
public class manage_MainForm extends JFrame {
	...
	private static final int WIDTH = 800;//设置整体界面的宽度
	private static final int HEIGHT = 600;//设置整体界面的高度
	...
	public manage_MainForm(User user,status sta) {
		this.currentUser = user;
		this.status = sta;
		init();		
	}
	private void init() {
		...
		this.setSize(WIDTH, HEIGHT);//设置主界面的大小
		this.setResizable(true);//设置主界面的大小不可调
	...
	JMenuBar menubar1=new JMenuBar();//设置一个菜单条
	    ...
	    menubar1.add(menu3);
		//设置子菜单项
	    JMenuItem item1=new JMenuItem("按ID查询",new ImageIcon("images/findid.png"));
		JMenuItem item2=new JMenuItem("按姓名查询",new ImageIcon("images/findname.png"));
		JMenuItem item3=new JMenuItem("按时间查询",new ImageIcon("images/findtime.png"));
		JMenuItem item4=new JMenuItem("按门号查询",new ImageIcon("images/findlimit.png"));
		JMenuItem item12=new JMenuItem("综合查询",new ImageIcon("images/findzonghe.png"));
		
		
		JMenuItem item5=new JMenuItem("用户基本信息",new ImageIcon("images/group.png"));
		
		JMenuItem item6=new JMenuItem("RFID制卡",new ImageIcon("images/card new.png"));
		JMenuItem item7=new JMenuItem("RFID卡挂失",new ImageIcon("images/cardlost.png"));
		JMenuItem item10=new JMenuItem("RFID卡删除",new ImageIcon("images/card delete.png"));
		JMenuItem item11=new JMenuItem("RFID卡修改",new ImageIcon("images/cardedit.png"));
		
		JMenuItem item8=new JMenuItem("门状态查询",new ImageIcon("images/door.png"));
		JMenuItem item9=new JMenuItem("退出系统",new ImageIcon("images/door_out.png"));
		JMenuItem item13=new JMenuItem("RFID制卡",new ImageIcon("images/add.png"));
		//往菜单项中添加子菜单
		menu1.add(item1);menu1.addSeparator();
		menu1.add(item2);menu1.addSeparator();
		menu1.add(item3);menu1.addSeparator();
		menu1.add(item4);menu1.addSeparator();
		menu1.add(item12);
		...
		pop.add(item8);pop.addSeparator();
		pop.add(item13);pop.addSeparator();
		pop.add(item9);
		
	    //创建一个panel
		pnlContent = UnitCommon.createPanel(false, new Dimension(WIDTH, 475),
		...
		item6.addActionListener(new ActionListener()
	      {
	          ...
	          {
	        	  pnlContent.removeAll();
	        	   card_register  a=new card_register();
	        	   pnlContent.add(a,BorderLayout.CENTER);
	        	   showDialog();
	          }
	      });
		 ...
		 item8.addActionListener(new ActionListener()
	      {
	          public void actionPerformed(ActionEvent Event) 
	          {
	        	   pnlContent.removeAll();
	        	   pnlContent.repaint();
	        	   pnlContent.add(status,BorderLayout.CENTER);
	        	   showDialog();
	          }
	      });
		 //右键退出菜单
